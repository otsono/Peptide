...
function thwompLanded() {
	...
	for (i in 0...objs.length) {
		var o = objs[i];
		if (Math.abs(o.getX() - px) < HALF_W && Math.abs(o.getY() - m_startY.get()) < 90) {
			return true;
		}
	}
	return false;
...
function update() {
	if (!m_init.get()) {
		...
		m_startX.set(self.getX());
	}
	var a = m_action.get();
	if (a == 0) { if (thwompLanded()) {
		m_action.set(1);
	} }
	else if (a == 1) { // sink + shake
		var sh = -1.3; if (m_shake.get()) {
			sh = 1.3;
		}
		self.setX(m_startX.get() + sh); m_shake.set(!m_shake.get());
		self.setY(self.getY() + SINK_SPEED);
		if (self.getY() >= m_startY.get() + SINK_DEPTH) {
			self.setY(m_startY.get() + SINK_DEPTH);
			self.setX(m_startX.get());
			m_action.set(2);
			m_timer.set(0);
		}
	} else if (a == 2) {
		...
		if (self.getY() <= m_startY.get()) {
			self.setY(m_startY.get());
			m_action.set(0);
		}
	}
}
