...
> **copyright / compliance boundary -- never publish.** at Team Fray's request, the
> tracked docs **don't explain how to decompile or patch the Fraymakers (HashLink) engine or
...
> FrayTools render internals). that material was removed and **must not** be re-added. it's
...
- **`peptide`** parses the engine's HashLink bytecode (`hlbc` crate), injects a per-frame
...
> access path). per Team Fray's request, that material -- specific non-hscript engine
