...
> workflow, both totally fine to document. what we leave out, to respect Team Fray's wishes,
...
the Fraymakers engine ships as compiled bytecode, and every update is a full recompile that
...
stops growing. the engine's bytecode is *monolithic* with no runtime module-load facility, so a
...
*local* engine at runtime, writes a patched *copy*, and speaks a loopback TCP protocol. to
respect Team Fray's wishes, the tracked docs don't name specific engine internals
