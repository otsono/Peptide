...
this `NOTICE.md` covers attribution to dependencies as required by their respective licenses.
...
| hlbc          | 0.7      | MIT -- HashLink bytecode reader/writer |
...
- **source code, bytecode, or disassembly** from FrayTools (`app.asar` / bundle JS), the
  Fraymakers engine (`hlboot-sdl.dat` / HashLink bytecode / Haxe source / `.hx:line` stack
...
**engine internals -- removed at Team Fray's request.** on top of the rule above
(never publish their *material*), Team Fray has asked that the tracked docs not
**explain how to decompile or patch** the Fraymakers (HashLink) engine binary or the FrayTools
...
that material has been **removed from the tracked docs and must not be re-added**; it lives
only in the user's local, gitignored `docs/` scratch space for interoperability work. what's
...
MIT is compatible with every dependency listed above (all are MIT, MIT/Apache-2.0, or carry
only attribution-style requirements that this NOTICE satisfies).
