...
/// Pivot transform (matches FrayTools' bundle math): rotate the `pivot`
/// offset vector around the origin by `angle_deg` and add to `pos`.
/// Exact-multiple-of-360 short-circuits to `pos + pivot` (FrayTools
/// special-cases this; we mirror it so our output is bit-for-bit aligned
/// with the no-rotation path).
